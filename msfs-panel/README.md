...
3. Copy `source/html_ui/InGamePanels/DispatchLinkEFB` into the sample's corresponding `html_ui/InGamePanels` asset directory.
4. Copy `config.js.example` to `config.js` and enter:
   - your deployed Render URL;
   - the same `SIM_LINK_TOKEN` configured on Render.
5. Point the sample panel's HTML entry to `DispatchLinkEFB/DispatchLinkEFB.html`.
6. Build the package in the MSFS Project Editor and install its built output in the Community folder.
7. Open the panel while in a flight. It sends a heartbeat every five seconds and displays the hosted DispatchLink EFB in the simulator panel.
