...
export default function App() {
  ...
  return <div className={`app-shell ${effectiveCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main>
      <header className="topbar"><button className="menu-button always-menu" onClick={() => { const landscapeTablet = window.matchMedia('(orientation: landscape) and (min-width: 700px)').matches; if (landscapeTablet || window.innerWidth > 1180) setSidebarCollapsed(value => !value); else setMenuOpen(true); }}><Menu /></button><div className="flight-ident"><div className="flight-primary"><span>{flight.airline}{flight.flightNumber || '—'}</span><strong>{flight.origin} <Plane size={16} /> {flight.destination}</strong></div><div className="flight-aircraft"><span><b>EQUIP</b>{flight.aircraft}</span><span><b>REG</b>{flight.registration}</span></div></div><div className="topbar-actions"><div className="flight-time-cluster"><div className="departure-clock" title="Time remaining until scheduled departure"><span>{departure.std}</span><strong><b>{departure.label}</b>{departure.value}</strong></div><div className="time-divider" aria-hidden="true"/><div className="zulu-clock"><span>ZULU</span><strong>{clock.toISOString().slice(11, 19)}</strong></div></div><button className="primary top-import" onClick={() => void importOFP()} disabled={loadingOFP}>{loadingOFP ? <RefreshCw className="spin" size={16} /> : <Import size={16} />}<span>{loadingOFP ? 'Syncing' : 'Import OFP'}</span></button></div></header>
