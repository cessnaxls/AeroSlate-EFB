...
function upgradeStages(profile: TrainingProfile): UpgradeStageDefinition[] {
  const lineHours = profile.operation === 'Part 135' ? 15 : 25;
  const upgradeHours = profile.operation === 'Part 121' ? 1000 : profile.operation === 'Part 135' ? 500 : profile.operation === 'EASA CAT' ? 500 : 250;
  if (profile.position === 'PIC') return [
    { key:'pic-initial', title:'PIC Initial Qualification', description:'Initial qualification on the aircraft in command.', defaultHours:0 },
    { key:'pic-oe', title:'PIC Operating Experience / Line Qualification', description:'Initial supervised operating experience and line qualification.', defaultHours:lineHours },
    { key:'pic-consolidation', title:'PIC Consolidation', description:'Consolidated operating experience in the aircraft after qualification.', defaultHours:100 }
  ];
  return [
    { key:'sic-initial', title:'SIC Initial Qualification', description:'Initial qualification on the aircraft as second-in-command.', defaultHours:0 },
    { key:'sic-oe', title:'SIC Operating Experience / Line Qualification', description:'Initial supervised operating experience and line qualification.', defaultHours:lineHours },
    { key:'sic-consolidation', title:'SIC Consolidation', description:'Consolidated line experience in the aircraft.', defaultHours:100 },
    { key:'pic-upgrade', title:'PIC Upgrade Eligibility', description:'Company planning threshold for consideration for PIC upgrade on this aircraft.', defaultHours:upgradeHours }
  ];
}
function resolvedUpgradeHours(profile: TrainingProfile, stage: UpgradeStageDefinition): number {
  const value = profile.upgrade?.hours?.[stage.key];
  return Number.isFinite(value) ? Number(value) : stage.defaultHours;
}
...
export function RecordsPage({ flight, mode = 'logbook' }: { flight: FlightSummary; mode?: 'logbook' | 'duty' }) {
  ...
  const upgradeStats = useMemo(()=>{
    if(!activeTraining) return null;
    const aircraftKey=activeTraining.aircraft.trim().toLowerCase();
    const matched=entries.filter(entry=>String(entry.data?.aircraftType||'').trim().toLowerCase()===aircraftKey);
    const roleField=activeTraining.position==='PIC'?'pic':'sic';
    const currentHours=matched.reduce((total,entry)=>total+Number(entry.data?.[roleField]||0),0);
    const stages=upgradeStages(activeTraining).map(stage=>{const required=resolvedUpgradeHours(activeTraining,stage);return {...stage,required,remaining:Math.max(0,required-currentHours),met:currentHours>=required};});
    const next=stages.find(stage=>!stage.met) || stages[stages.length-1];
    return {currentHours,stages,next,flights:matched.filter(entry=>Number(entry.data?.[roleField]||0)>0).length};
  },[activeTraining,entries]);
  const updateUpgradeHours=(stageKey:string,value:number)=>{if(!activeTraining)return;setTrainingProfiles(rows=>rows.map(profile=>profile.id===activeTraining.id?{...profile,upgrade:{hours:{...(profile.upgrade?.hours||{}),[stageKey]:Math.max(0,value)}}}:profile));};
  const resetUpgradeHours=()=>{if(!activeTraining)return;setTrainingProfiles(rows=>rows.map(profile=>profile.id===activeTraining.id?{...profile,upgrade:undefined}:profile));setStatus('Aircraft upgrade thresholds reset to defaults.');};
  ...
  return <div className="records-page">
    ...
    {tab === 'logbook' && <section className="card aircraft-upgrade-card"><header><div><Plane size={18}/><h3>Aircraft upgrade ladder</h3></div><span className="pill blue">PER AIRCRAFT · HOURS</span></header><div className="card-body">
      {activeTraining&&upgradeStats?<>
        <div className="upgrade-identity"><div><strong>{activeTraining.airline} · {activeTraining.aircraft}</strong><span>{activeTraining.operation} · tracking {activeTraining.position} time in type</span></div><button className="compact" onClick={resetUpgradeHours}><RefreshCw size={13}/> Reset defaults</button></div>
        <div className="upgrade-summary"><div><span>Current {activeTraining.position} time</span><strong>{upgradeStats.currentHours.toFixed(1)} hr</strong><small>{upgradeStats.flights} qualifying flight{upgradeStats.flights===1?'':'s'} in {activeTraining.aircraft}</small></div><div><span>Next threshold</span><strong>{upgradeStats.next?.title||'—'}</strong><small>{upgradeStats.next?.met?'All configured thresholds met':`${upgradeStats.next?.remaining.toFixed(1)} hr remaining`}</small></div></div>
        <div className="upgrade-table"><div className="upgrade-table-head"><span>Status</span><span>Qualification / progression point</span><span>Hours required</span><span>Current</span><span>Remaining</span></div>{upgradeStats.stages.map(stage=><div className={`upgrade-row ${stage.met?'met':'pending'}`} key={stage.key}><div><span className={`pill ${stage.met?'good':'neutral'}`}>{stage.met?'MET':'PENDING'}</span></div><div className="upgrade-stage-label"><strong>{stage.title}</strong><small>{stage.description}</small></div><label className="upgrade-hours-input"><input type="number" min="0" step="1" value={stage.required} onChange={e=>updateUpgradeHours(stage.key,Number(e.target.value)||0)}/><span>hr</span></label><strong>{upgradeStats.currentHours.toFixed(1)}</strong><strong>{stage.met?'—':stage.remaining.toFixed(1)}</strong></div>)}</div>
        <p className="currency-disclaimer">Titles use normal airline/operator terminology. Hour values are editable planning thresholds; the defaults are not represented as FAA, EASA, or company-mandated upgrade minimums.</p>
      </>:<div className="training-empty"><Plane size={22}/><div><strong>Select or create an aircraft training record</strong><p>The upgrade ladder is generated automatically for that aircraft and crew position. You only adjust the hour thresholds.</p></div></div>}
    </div></section>}
