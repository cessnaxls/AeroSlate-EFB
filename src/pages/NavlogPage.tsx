  ...
  return <section className="card navlog-card">
    ...
    <div className="card-body compact-card-body">
      ...
      <div className={`table-scroll navlog-scroll axis-${scrollAxis}`}><table className={`navlog-table rich-navlog ${active ? 'active-mode' : ''}`}><thead><tr>
        <th>#</th><th>Fix</th><th>Via</th><th>CRS</th><th>Dist</th><th>ALT</th><th>SPD</th><th>Wind / OAT / ISA</th><th>LEG / ETA</th><th>Fuel</th>
        ...
        {fixes.map(({ fix, index, legFuel, plannedRemaining, plannedElapsedMinutes, plannedEta }) => {
          ...
          return <tr key={id} className={actual.complete ? 'navlog-complete' : ''}>
            ...
            <td>{fix.wind_dir ? `${String(fix.wind_dir).padStart(3, '0')}/${fix.wind_spd || 0}` : '—'}<small>{field(fix, 'oat') !== undefined ? `OAT ${field(fix, 'oat')}°C` : ''}</small><small className="isa-dev">{field(fix, 'isa_dev', 'isa_deviation', 'isa') !== undefined ? `ISA ${Number(field(fix, 'isa_dev', 'isa_deviation', 'isa')) > 0 ? '+' : ''}${field(fix, 'isa_dev', 'isa_deviation', 'isa')}°C` : ''}</small></td>
