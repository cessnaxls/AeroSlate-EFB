...
export function SimBriefDispatchPage({ url, flight, staticId, loading, onImport }: Props) {
  ...
  return <div className="simbrief-workspace">
    <section className="card dispatch-control-card">
      ...
      <div className="card-body">
        <div className="dispatch-steps">
          ...
          <div><span>3</span><strong>Automatic sync</strong><small>DispatchLink watches this flight ID and copies the generated OFP into every module.</small></div>
        </div>
    ...
    <section className="card provider-card"><ProviderPortal title="SimBrief Dispatch" url={url} windowName="dispatchlink-simbrief" description="The native DispatchLink app keeps the authenticated SimBrief session inside the app. Generate the OFP here; the flight watcher then imports it without closing DispatchLink." /></section>
  </div>;
}
