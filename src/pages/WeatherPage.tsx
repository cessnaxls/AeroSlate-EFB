...
import { AlertTriangle, ChevronDown, CloudSun, ExternalLink, Filter, Wifi, RefreshCw, Search, ShieldCheck } from 'lucide-react';
...
export function WeatherPage({ofp,flight}:{ofp:AnyRecord|null;flight:FlightSummary}){
  ...
  useEffect(()=>{void loadAtis()},[flight.origin,flight.destination]);
  ...
  return <div className="weather-page foreflight-briefing">
    <section className="card atis-collapsible"><details><summary><div><Wifi size={18}/><span><strong>D-ATIS</strong><small>Real-world and VATSIM airport information</small></span></div><div className="summary-actions"><button onClick={(event)=>{event.preventDefault();void loadAtis()}} disabled={atisBusy}><RefreshCw className={atisBusy?'spin':''} size={15}/>Refresh</button><ChevronDown size={17}/></div></summary><div className="card-body atis-grid">{stations.slice(0,2).map(s=>{const data=atis[s.code];return <div className="atis-station" key={s.code}><strong>{s.code}</strong><div><span>Real-world</span><p>{data?.realWorld?.[0]?.text||'No current public D-ATIS returned.'}</p></div><div><span>VATSIM</span><p>{data?.vatsim?.[0]?.text||'No VATSIM ATIS online.'}</p></div></div>})}<small>Public D-ATIS/ACARS messages may be delayed. VATSIM ATIS is shown separately.</small></div></details></section>
