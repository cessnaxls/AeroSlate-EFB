...
export function ChartsPage({ ofp, flight, source, setSource }: Props) {
  ...
  return <div className="charts-page">
    ...
    {view === 'navigraph' && <section className="card provider-card charts-provider"><ProviderPortal title="Navigraph Charts" url="https://charts.navigraph.com/" windowName="aeroslate-navigraph" description="The official authenticated Navigraph Charts workspace is kept inside AeroSlate’s native provider frame." /></section>}
    {view === 'binder' && <div className="binder-layout"><aside className="card binder-sidebar"><header><div><FileText size={18} /><h3>Flight documents</h3></div></header><div className="card-body binder-links">
    ...
    </div></aside><div className="binder-workspace"><ChartWorkspace source={source} /></div></div>}
  </div>;
}
